...
int main(int argc,char ** argv) try {
    ...
    std::unordered_map<ENetPeer*,Player> players;std::vector<Projectile> projectiles;net::PlayerId next_id=1;std::uint32_t next_projectile_id=1,next_sound_event_id=1; std::uint32_t tick=0; bool running=true; net::RoomState room=net::RoomState::lobby; net::SongSchedule song{};
    ...
    while(running){
        ...
        for(auto it=projectiles.begin();it!=projectiles.end();){const Vec2f next=it->position+it->velocity*dt;bool destroy=now-it->spawn_us>=kProjectileLifetimeUs||collision.blocks_segment(it->position,next);if(!destroy){for(auto & [peer,p]:players){(void)peer;if(!p.active||!p.alive||p.id==it->owner_id||now<p.protected_until_us)continue;if(segment_distance_squared(it->position,next,p.position)<(kProjectileRadius+kPlayerRadius)*(kProjectileRadius+kPlayerRadius)){destroy=true;if(now<p.shield_until_us){emit_sound(players,collision,next_sound_event_id,net::SoundCue::shot_hit_shield,it->owner_id,p.id,p.position,{it->owner_id,p.id});}else{p.alive=false;p.velocity={};p.input_x=p.input_y=0;p.respawn_at_us=now+kRespawnDelayUs;p.protected_until_us=0;p.shield_until_us=0;emit_sound(players,collision,next_sound_event_id,net::SoundCue::death,it->owner_id,p.id,p.position,{it->owner_id,p.id});}break;}}}if(destroy)it=projectiles.erase(it);else{it->position=next;++it;}}
