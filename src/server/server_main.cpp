...
int main(int argc,char ** argv) try {
    ...
    while(running){
        ...
        for(auto it=projectiles.begin();it!=projectiles.end();){const Vec2f next=it->position+it->velocity*dt;bool destroy=now-it->spawn_us>=kProjectileLifetimeUs||collision.blocks_segment(it->position,next);if(!destroy){for(auto & [peer,p]:players){(void)peer;if(!p.active||!p.alive||p.id==it->owner_id||now<p.protected_until_us)continue;if(segment_distance_squared(it->position,next,p.position)<(kProjectileRadius+kPlayerRadius)*(kProjectileRadius+kPlayerRadius)){destroy=true;if(now>=p.shield_until_us){p.alive=false;p.velocity={};p.input_x=p.input_y=0;p.respawn_at_us=now+kRespawnDelayUs;p.protected_until_us=0;p.shield_until_us=0;}break;}}}if(destroy)it=projectiles.erase(it);else{it->position=next;++it;}}
