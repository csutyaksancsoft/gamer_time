...
int main(int argc,char ** argv) try {
    ...
    std::uint8_t team_count=2;bool friendly_fire=false;
    for(int i=1;i<argc;++i){const std::string arg=argv[i];if(arg=="--bind"&&i+1<argc)bind=argv[++i];else if(arg=="--map"&&i+1<argc)map_path=argv[++i];else if(arg=="--song"&&i+1<argc)song_path=argv[++i];else if(arg=="--team-count"&&i+1<argc){const int n=std::stoi(argv[++i]);if(n<2||n>4)fail("--team-count must be 2-4");team_count=static_cast<std::uint8_t>(n);}else if(arg=="--friendly-fire"&&i+1<argc){const std::string v=argv[++i];if(v!="on"&&v!="off")fail("--friendly-fire must be on or off");friendly_fire=v=="on";}else fail("Usage: gamer_time_server [--bind host:port] [--map path] [--song path] [--team-count 2-4] [--friendly-fire on|off]");}
    ...
    while(running){
        ENetEvent event{}; while(enet_host_service(host,&event,0)>0){
            if(event.type==ENET_EVENT_TYPE_DISCONNECT){auto found=players.find(event.peer);if(found!=players.end()){std::cout<<found->second.name<<" disconnected\n";vote.disconnect(found->second.id);players.erase(found);}}
                ...
                try{net::Reader reader({event.packet->data,event.packet->dataLength});const auto type=reader.type();auto found=players.find(event.peer);
                    ...
                    else if(found!=players.end()&&type==net::MessageType::team_request){const auto team=net::read_team_request(reader);if(!net::team_switching_allowed(room)||!net::valid_team(team,team_count))fail("Team selection rejected");found->second.team=team;}
        ...
        const auto now=net::monotonic_time_us();if(room==net::RoomState::voting&&vote.complete(now)){mode=vote.result();vote.finish();song={now+3000000ULL,song_config.duration_ms,song_config.bpm,song_config.first_beat_ms,song_config.subdivision,song_config.id,chart};room=net::RoomState::countdown;for(auto &[peer,p]:players){p.round_kills=p.round_deaths=0;p.score={};p.judged_notes.assign(chart.size(),0);p.next_expiring_note=0;p.active=true;send_packet(peer,net::make_song_schedule(song),true);}std::cout<<"Vote resolved: "<<(mode==net::GameMode::teams?"Teams":"FFA")<<"; song scheduled in 3 seconds\n";}if(room==net::RoomState::countdown&&now>=song.server_start_us)room=net::RoomState::playing;
        ...
        if(tick%3==0){net::Snapshot snapshot{};snapshot.server_time_us=now;snapshot.server_tick=tick;snapshot.room=room;snapshot.mode=mode;snapshot.vote_deadline_us=vote.active()?vote.deadline_us():0;snapshot.eligible_voters=vote.active()?vote.eligible_count():0;snapshot.teams_votes=vote.active()?vote.teams_count():0;snapshot.ffa_votes=vote.active()?vote.ffa_count():0;snapshot.team_count=team_count;snapshot.friendly_fire=friendly_fire;for(const auto & [peer,p]:players){(void)peer;if(p.active){net::PlayerState state{};state.id=p.id;state.position=p.position;state.velocity=p.velocity;state.acknowledged_input=p.input_sequence;state.color=p.color;state.name=p.name;state.alive=p.alive;state.facing_angle=p.facing_angle;state.respawn_at_us=p.respawn_at_us;state.protected_until_us=p.protected_until_us;state.shield_until_us=p.shield_until_us;state.team=p.team;state.vote=vote.active()?vote.choice(p.id):net::VoteChoice::none;state.round_kills=p.round_kills;state.round_deaths=p.round_deaths;state.session_kills=p.session_kills;state.session_deaths=p.session_deaths;snapshot.players.push_back(state);}}for(const auto & p:projectiles)snapshot.projectiles.push_back({p.id,p.owner_id,p.position,p.velocity,p.angle});const auto bytes=net::make_snapshot(snapshot);for(auto & [peer,p]:players){(void)p;send_packet(peer,bytes,false);}}
        if(terminal_line_ready()){std::string line;std::getline(std::cin,line);if(line=="quit")running=false;else if(line=="status")std::cout<<players.size()<<" players, tick "<<tick<<", room "<<static_cast<int>(room)<<"\n";else if(line=="stop")room=net::RoomState::free_move;else if(line=="start"){const bool all_ready=std::all_of(players.begin(),players.end(),[](const auto & entry){return entry.second.ready;});if(players.empty()||!all_ready||room==net::RoomState::voting||room==net::RoomState::countdown||room==net::RoomState::playing)std::cout<<"Start refused: all connected clients must be ready and the room idle\n";else{std::vector<net::PlayerId> eligible;for(const auto&[peer,p]:players){(void)peer;if(p.active)eligible.push_back(p.id);}vote.begin(eligible,now);room=net::RoomState::voting;std::cout<<"Mode vote started for 15 seconds\n";}}else if(line.rfind("kick ",0)==0){const auto id=static_cast<net::PlayerId>(std::stoul(line.substr(5)));for(auto & [peer,p]:players)if(p.id==id){enet_peer_disconnect(peer,0);break;}}}
