...
bool parse_endpoint(std::string_view text,std::string & host,std::uint16_t & port){host.clear();port=net::kDefaultPort;if(text.empty()||text.find_first_of(" \t\r\n/")!=std::string_view::npos)return false;auto colon=text.rfind(':');if(colon==std::string_view::npos){host=std::string(text);return true;}if(colon==0||colon+1==text.size()||text.find(':')!=colon)return false;host=std::string(text.substr(0,colon));unsigned value=0;const auto part=text.substr(colon+1);const auto result=std::from_chars(part.data(),part.data()+part.size(),value);if(result.ec!=std::errc{}||result.ptr!=part.data()+part.size()||value<1||value>65535)return false;port=static_cast<std::uint16_t>(value);return true;}
