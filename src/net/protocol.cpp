...
std::vector<std::uint8_t> make_song_schedule(const SongSchedule & s) { Writer w(MessageType::song_schedule); w.u64(s.server_start_us); w.u32(s.duration_ms); w.f32(s.bpm); w.i32(s.first_beat_ms); w.u16(s.subdivision); return w.bytes(); }
SongSchedule read_song_schedule(Reader & r) { SongSchedule s{}; s.server_start_us=r.u64(); s.duration_ms=r.u32(); s.bpm=r.f32(); s.first_beat_ms=r.i32(); s.subdivision=r.u16(); return s; }
...
std::vector<std::uint8_t> make_rhythm_result(const RhythmResult & r) { Writer w(MessageType::rhythm_result); w.u8(static_cast<std::uint8_t>(r.grade)); w.i32(r.offset_ms); w.u32(r.perfect); w.u32(r.good); w.u32(r.miss); return w.bytes(); }
RhythmResult read_rhythm_result(Reader & r) { RhythmResult v{}; v.grade=static_cast<RhythmGrade>(r.u8()); v.offset_ms=r.i32(); v.perfect=r.u32(); v.good=r.u32(); v.miss=r.u32(); return v; }
