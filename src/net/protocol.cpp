...
std::vector<std::uint8_t> make_snapshot(const Snapshot & s) {
    ...
    w.u8(static_cast<std::uint8_t>(s.room));w.u8(static_cast<std::uint8_t>(s.mode));w.u64(s.vote_deadline_us);w.u8(s.eligible_voters);w.u8(s.teams_votes);w.u8(s.ffa_votes);w.u8(s.team_count);w.u8(s.friendly_fire?1:0);
    return w.bytes();
}
