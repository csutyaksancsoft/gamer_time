...
std::vector<std::uint8_t> make_snapshot(const Snapshot & s) {
    ...
    w.u8(static_cast<std::uint8_t>(s.room));w.u8(static_cast<std::uint8_t>(s.mode));w.u64(s.vote_deadline_us);w.u8(s.eligible_voters);w.u8(s.teams_votes);w.u8(s.ffa_votes);w.u8(s.team_count);w.u8(s.friendly_fire?1:0);w.u8(s.mode_vote_enabled?1:0);w.u8(s.song_vote_enabled?1:0);const auto songs=std::min(s.song_candidates.size(),kMaxSongCandidates);w.u8(static_cast<std::uint8_t>(songs));for(std::size_t i=0;i<songs;++i){w.string(s.song_candidates[i],64);w.u8(i<s.song_votes.size()?s.song_votes[i]:0);}w.string(s.selected_song_id,64);
    return w.bytes();
}
...
std::vector<std::uint8_t> make_rhythm_result(const RhythmResult & r) { Writer w(MessageType::rhythm_result); w.u8(static_cast<std::uint8_t>(r.grade)); w.i32(r.offset_ms); w.u32(r.perfect); w.u32(r.good); w.u32(r.miss); w.u32(r.note_index); w.u32(r.combo); w.u32(r.max_combo); w.u8(r.overstrum?1:0);w.u8(r.shot_fired?1:0);w.u8(r.shield_activated?1:0);return w.bytes(); }
RhythmResult read_rhythm_result(Reader & r) { RhythmResult v{}; v.grade=static_cast<RhythmGrade>(r.u8()); v.offset_ms=r.i32(); v.perfect=r.u32(); v.good=r.u32(); v.miss=r.u32(); v.note_index=r.u32(); v.combo=r.u32(); v.max_combo=r.u32(); v.overstrum=r.u8()!=0;v.shot_fired=r.u8()!=0;v.shield_activated=r.u8()!=0;return v; }
