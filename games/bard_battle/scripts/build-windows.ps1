...
if (-not (Test-Path $sdlCMake)) {
    ...
    $temporaryPath = Join-Path ([System.IO.Path]::GetTempPath()) ("bard-battle-sdl-" + [guid]::NewGuid())
...
if (-not (Test-Path $enetCMake)) {
    ...
    $temporaryPath = Join-Path ([System.IO.Path]::GetTempPath()) ("bard-battle-enet-" + [guid]::NewGuid())
...
& cmake `
    ...
    -A x64 `
    -DATTEMPT_ENGINE_BUILD_BARD_BATTLE=ON `
    -DBARD_BATTLE_BUILD_CLIENT=ON `
    -DBARD_BATTLE_BUILD_SERVER=OFF `
    -DBUILD_TESTING=OFF
Assert-LastExitCode "CMake configuration"

Write-Host "Building $Configuration client..." -ForegroundColor Cyan
& cmake --build $buildPath --config $Configuration --target bard_battle --parallel
Assert-LastExitCode "$Configuration build"

$executable = Join-Path $buildPath "games\bard_battle\$Configuration\bard_battle.exe"
...
if (-not $SkipPackage) {
    ...
    $package = Join-Path $buildPath "bard-battle-windows-x64.zip"
