...
cmake -S "$repo_root" -B "$repo_root/build-server" \
    -DATTEMPT_ENGINE_BUILD_BARD_BATTLE=ON -DBARD_BATTLE_BUILD_CLIENT=OFF -DBARD_BATTLE_BUILD_SERVER=ON -DCMAKE_BUILD_TYPE=Release
cmake --build "$repo_root/build-server" --parallel

echo "Server: $repo_root/build-server/games/bard_battle/bard_battle_server"
echo "Bots:   $repo_root/build-server/games/bard_battle/bard_battle_bot"
