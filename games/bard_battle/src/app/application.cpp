...
void Application::initialize() {
    ...
    platform_.initialize({"Bard Battle", config_.initial_width, config_.initial_height});
    ...
    effect_player_.load(audio_device_,config_.asset_dir + "/audio/sfx");
    initialized_ = true;
}
...
void Application::tick_frame(float dt_seconds) {
    ...
    while(network_.take_sound_event(sound_event)){
        effect_player_.play(sound_event.cue,predicted_position_,sound_event.position,sound_event.participant);
        net::PlayerId animated=0;entity_animation::PlayerAnimation state=entity_animation::PlayerAnimation::idle;
        if(sound_event.cue==net::SoundCue::shot_success){animated=sound_event.source_id;state=entity_animation::PlayerAnimation::attack;}
    ...
    if (!snapshot.players.empty()) {
        ...
        units.reserve(snapshot.players.size()+snapshot.projectiles.size()+snapshot.melee_effects.size());
        UnitId render_id=0;
        ...
        for (const net::PlayerState & player : snapshot.players) {
            ...
            const auto animation_event=player_animation_events_.find(player.id);const bool event_active=animation_event!=player_animation_events_.end()&&snapshot.server_time_us<animation_event->second.ends_us;
            if(!player.alive&&(!event_active||animation_event->second.animation!=entity_animation::PlayerAnimation::death))continue;
            Vec2f position = player.position;
            if(!player.alive&&event_active)position=animation_event->second.position;
            if (player.alive && player.id == network_.player_id()) {
            ...
            std::uint64_t animation_elapsed=0;bool spinning_melee=false;
            if(const auto*strip=entity_animations_.find_player(folder,state)){animation_elapsed=event_active?(snapshot.server_time_us>=animation_event->second.started_us?snapshot.server_time_us-animation_event->second.started_us:0):shielding&&snapshot.server_time_us>=player.shield_started_us?snapshot.server_time_us-player.shield_started_us:snapshot.server_time_us;const bool loop=state==entity_animation::PlayerAnimation::running||state==entity_animation::PlayerAnimation::shield;const auto frame=entity_animation::frame_index(*strip,animation_elapsed,loop);unit.sprite_index=strip->frames[frame].atlas_index;unit.atlas_span=static_cast<std::uint32_t>(strip->atlas_span_x)|(static_cast<std::uint32_t>(strip->atlas_span_y)<<16);unit.size=strip->world_size;spinning_melee=state==entity_animation::PlayerAnimation::melee&&entity_animations_.find_player_exact(folder,state)==strip;}
            ...
            if(spinning_melee){const auto duration=entity_animation::duration_us(*entity_animations_.find_player_exact(folder,state));unit.rotation_radians=entity_animation::melee_rotation_radians(animation_elapsed,duration);}
            else{if(player_faces_left_[player.id])unit.transform_flags=assets::kTmxFlipHorizontal;unit.rotation_radians=0.0f;}
        ...
        for(const net::ProjectileState & p:snapshot.projectiles){ReplicatedUnitState unit{};unit.id=++render_id;unit.position=p.position;unit.sprite_index=1000001u;unit.size={18.0f,6.0f};unit.rotation_radians=p.angle;if(const auto*strip=entity_animations_.find(entity_animation::Role::projectile)){const auto elapsed=snapshot.server_time_us>=p.spawned_at_us?snapshot.server_time_us-p.spawned_at_us:0;unit.sprite_index=strip->frames[entity_animation::frame_index(*strip,elapsed,true)].atlas_index;unit.atlas_span=static_cast<std::uint32_t>(strip->atlas_span_x)|(static_cast<std::uint32_t>(strip->atlas_span_y)<<16);unit.size=strip->world_size;}else{unit.solid_color=true;unit.color[0]=1.0f;unit.color[1]=0.85f;unit.color[2]=0.15f;}units.push_back(unit);}
        for(const net::MeleeEffectState & melee:snapshot.melee_effects){if(melee.expires_at_us<=snapshot.server_time_us)continue;const auto owner=std::find_if(snapshot.players.begin(),snapshot.players.end(),[&](const auto&p){return p.id==melee.owner_id;});if(owner!=snapshot.players.end()){const auto folder=entity_animation::player_folder(owner->id,owner->team,snapshot.mode==net::GameMode::teams);if(entity_animations_.find_player_exact(folder,entity_animation::PlayerAnimation::melee))continue;}ReplicatedUnitState unit{};unit.id=++render_id;unit.position=melee.position;unit.size={melee.radius*2.0f,melee.radius*2.0f};unit.rotation_radians=melee.angle;if(const auto*strip=entity_animations_.find(entity_animation::Role::melee)){const auto elapsed=snapshot.server_time_us>=melee.started_at_us?snapshot.server_time_us-melee.started_at_us:0;unit.sprite_index=strip->frames[entity_animation::frame_index(*strip,elapsed,false)].atlas_index;unit.atlas_span=static_cast<std::uint32_t>(strip->atlas_span_x)|(static_cast<std::uint32_t>(strip->atlas_span_y)<<16);unit.size=strip->world_size;}else{unit.solid_color=true;unit.circle_outline=true;unit.color[0]=1.0f;unit.color[1]=0.35f;unit.color[2]=0.1f;unit.color[3]=0.82f;}units.push_back(unit);}
        world_.replace_replicated_units(units);
        world_.set_local_unit(network_.player_id());
    }
    for(auto it=player_animation_events_.begin();it!=player_animation_events_.end();)if(snapshot.server_time_us>=it->second.ends_us)it=player_animation_events_.erase(it);else ++it;
    ...
    const std::uint64_t predicted_server_now=network_.server_time_us();
    const bool stunned=local_player!=snapshot.players.end()&&local_player->stunned_until_us>predicted_server_now;
    const bool shield_movement_locked=local_player!=snapshot.players.end()&&snapshot.shield_freeze&&local_player->shield_until_us>predicted_server_now;
    Vec2f movement{};
    ...
    movement = local_alive_&&!stunned&&!shield_movement_locked?normalize_or_zero(movement):Vec2f{};
    ...
    if(input.left_mouse_pressed&&!ui_consumed&&local_alive_&&!stunned){const Vec2f target=screen_to_world(input.mouse_x,input.mouse_y,input.window_width,input.window_height,camera_controller_.state());const Vec2f aim=normalize_or_zero(target-predicted_position_);rhythm_hud_.predict_hit(rhythm_now_us,config_.calibration_ms);network_.send_rhythm_hit(config_.calibration_ms,aim,net::RhythmAction::shoot);}
    else if(input.right_mouse_pressed&&local_alive_&&!stunned){rhythm_hud_.predict_hit(rhythm_now_us,config_.calibration_ms);network_.send_rhythm_hit(config_.calibration_ms,{},net::RhythmAction::shield);}
    else if(input.space_pressed&&local_alive_&&!stunned){const Vec2f target=screen_to_world(input.mouse_x,input.mouse_y,input.window_width,input.window_height,camera_controller_.state());const Vec2f aim=normalize_or_zero(target-predicted_position_);rhythm_hud_.predict_hit(rhythm_now_us,config_.calibration_ms);network_.send_rhythm_hit(config_.calibration_ms,aim,net::RhythmAction::melee);}
    ...
    if(network_.take_song_schedule(schedule)) {
        ...
        if(config&&song_player_.load(audio_device_,config_.asset_dir+"/audio/songs",*config))song_player_.schedule(local_start);
...
std::string Application::build_overlay_text() const {
    ...
    overlay << "BARD BATTLE NETWORK ARENA\n";
    overlay << "ESC quit | WASD move | LEFT CLICK fire | RIGHT CLICK shield | SPACE melee | wheel zoom | F3 collision | F4 solid terrain | F5 text\n\n";
    ...
    overlay << "Audio device: " << (audio_device_.ready() ? "ready" : audio_device_.diagnostic()) << '\n';
    overlay << "Music: " << (song_player_.error().empty() ? "ready" : song_player_.error()) << '\n';
    overlay << "SFX: " << effect_player_.loaded_clip_count() << " clips in " << effect_player_.populated_pool_count() << "/10 cues"
            << " | events " << effect_player_.request_count() << " | queued " << effect_player_.queued_count()
            << " | last " << effect_player_.last_cue() << '\n';
    if(!effect_player_.diagnostic().empty())overlay<<"SFX status: "<<effect_player_.diagnostic()<<'\n';
    if(have_rhythm_result_) overlay << "Last hit: " << net::grade_name(last_rhythm_result_.grade) << " (" << last_rhythm_result_.offset_ms << " ms) | "<<(last_rhythm_result_.shot_fired?"SHOT":last_rhythm_result_.shield_activated?"SHIELD":last_rhythm_result_.melee_fired?"MELEE":"NO ACTION")<<" | P/G/M " << last_rhythm_result_.perfect << "/" << last_rhythm_result_.good << "/" << last_rhythm_result_.miss << '\n';
