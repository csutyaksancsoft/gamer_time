...
int main(int argc,char ** argv) try {
    ...
    while(running){
        ...
        for(auto & [peer,player]:players){if(!player.active||!player.alive)continue;Vec2f direction=normalize_or_zero({player.input_x/127.0f,player.input_y/127.0f});player.velocity=direction*net::kMoveSpeed;Vec2f candidate=player.position+player.velocity*dt;if(collision.blocks_segment(CollisionChannel::Player,player.position,candidate)){candidate=player.position;player.velocity={};}
