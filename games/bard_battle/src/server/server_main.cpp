...
bool can_observe(const Player & observer,Vec2f position,const CollisionWorld & collision){return observer.active&&observer.alive&&length_squared(position-observer.position)<=net::kVisionRadius*net::kVisionRadius&&!collision.blocks_segment(CollisionChannel::Vision,observer.position,position);}
...
int main(int argc,char ** argv) try {
    ...
    const MapWorld map=MapWorld::from_tmx(assets::load_tmx_map(config.map)); const CollisionWorld collision=CollisionWorld::from_map(map);const server::SpawnZones spawn_zones(map,collision);
    ...
    std::unordered_map<ENetPeer*,Player> players;std::vector<Projectile> projectiles;std::vector<net::MeleeEffectState> melee_effects;net::PlayerId next_id=1;std::uint32_t next_projectile_id=1,next_melee_id=1,next_sound_event_id=1; std::uint32_t tick=0; bool running=true; net::RoomState room=net::RoomState::lobby;net::GameMode mode=settings.mode;game::ModeVote vote;game::SongVote song_vote;net::SongSchedule song{};server::RoundConfig active_round=server::defaults_for(settings,catalog.songs().size());std::vector<std::string> candidates;std::string selected_song;std::mt19937 random{std::random_device{}()};
    auto spawn_player=[&](const Player * excluded,net::TeamId team){std::vector<server::SpawnOccupant> occupied;for(const auto &[peer,p]:players){(void)peer;if(&p!=excluded&&p.active&&p.alive)occupied.push_back({p.position,true});}return spawn_zones.sample(mode,team,random,occupied);};
    auto schedule_song=[&](const std::string&id,std::uint64_t at){const auto*cfg=catalog.find(id);if(!cfg)fail("Selected song vanished from catalog: "+id);const auto chart=generate_beat_grid(*cfg);song={at+3000000ULL,cfg->duration_ms,cfg->bpm,cfg->first_beat_ms,cfg->subdivision,cfg->id,chart};selected_song=id;room=net::RoomState::countdown;projectiles.clear();melee_effects.clear();for(auto &[peer,p]:players){p.round_kills=p.round_deaths=0;p.score={};p.judged_notes.assign(chart.size(),0);p.next_expiring_note=0;p.active=true;p.alive=true;p.position=spawn_player(&p,p.team);p.velocity={};p.input_x=p.input_y=0;p.shield_until_us=0;p.stunned_until_us=0;p.respawn_at_us=0;p.protected_until_us=0;send_packet(peer,net::make_song_schedule(song),true);}std::cout<<"Selected "<<id<<"; countdown started\n";};
        ...
        ENetEvent event{}; while(enet_host_service(host,&event,0)>0){
            if(event.type==ENET_EVENT_TYPE_DISCONNECT){auto found=players.find(event.peer);if(found!=players.end()){std::cout<<found->second.name<<" disconnected\n";vote.disconnect(found->second.id);song_vote.disconnect(found->second.id);players.erase(found);}}
                ...
                try{net::Reader reader({event.packet->data,event.packet->dataLength});const auto type=reader.type();auto found=players.find(event.peer);
                    ...
                    else if(found!=players.end()&&type==net::MessageType::rhythm_hit){
                        reader.u32();const auto hit=reader.u64();const auto calibration=std::clamp<int>(reader.i16(),-250,250);Vec2f aim={reader.f32(),reader.f32()};const auto action=static_cast<net::RhythmAction>(reader.u8());
                        if(action!=net::RhythmAction::shoot&&action!=net::RhythmAction::shield&&action!=net::RhythmAction::melee)fail("Invalid rhythm action");
                        const auto action_now=net::monotonic_time_us();net::RhythmResult result=found->second.score;result.shot_fired=false;result.shield_activated=false;result.melee_fired=false;
                        const bool action_locked=game::actions_locked(action_now,found->second.stunned_until_us);
                        if(room==net::RoomState::playing&&found->second.alive&&!action_locked)result=grade_hit(found->second,hit,static_cast<std::int16_t>(calibration),song);
                        else if(!action_locked){result.grade=net::RhythmGrade::miss;++result.miss;result.combo=0;found->second.score=result;}
                        const float aim_length=length(aim);const bool valid_aim=std::isfinite(aim.x)&&std::isfinite(aim.y)&&aim_length>0.999f&&aim_length<1.001f;
                        const bool successful=!action_locked&&result.grade!=net::RhythmGrade::miss&&found->second.alive&&room==net::RoomState::playing;
                        result.shot_fired=successful&&action==net::RhythmAction::shoot&&valid_aim;result.shield_activated=successful&&action==net::RhythmAction::shield;result.melee_fired=successful&&action==net::RhythmAction::melee&&valid_aim;
                        const auto beat_us=game::beat_duration_us(song.bpm);
                        if(result.shield_activated){found->second.shield_started_us=action_now;found->second.shield_until_us=action_now+beat_us;}
                        if(result.shot_fired){aim=aim/aim_length;found->second.facing_angle=std::atan2(aim.y,aim.x);projectiles.push_back({next_projectile_id++,found->second.id,found->second.position+aim*14.0f,aim*kProjectileSpeed,action_now,found->second.facing_angle});}
                        if(result.melee_fired){
                            aim=aim/aim_length;found->second.facing_angle=std::atan2(aim.y,aim.x);const Vec2f center=found->second.position+aim*game::kMeleeOffset;melee_effects.push_back({next_melee_id++,found->second.id,center,game::kMeleeRadius,action_now+game::kMeleeEffectUs,action_now,found->second.facing_angle});emit_sound(players,collision,next_sound_event_id,net::SoundCue::swing_attack,found->second.id,0,found->second.position,{found->second.id});
                            for(auto & [target_peer,target]:players){(void)target_peer;if(!target.active||!target.alive||target.id==found->second.id||action_now<target.protected_until_us)continue;if(!game::projectile_can_hit(mode,settings.friendly_fire,found->second.team,target.team))continue;if(!game::melee_overlaps(center,game::kMeleeRadius,target.position,kPlayerRadius)||collision.blocks_segment(CollisionChannel::Shot,found->second.position,target.position))continue;
                                const bool shielded=action_now<target.shield_until_us;if(shielded){emit_sound(players,collision,next_sound_event_id,net::SoundCue::shield_break,found->second.id,target.id,target.position,{found->second.id,target.id});const auto resolution=game::resolve_shield_melee(settings.shield_melee,action_now,beat_us,target.stunned_until_us);if(!resolution.killed){target.shield_until_us=resolution.shield_until;target.stunned_until_us=resolution.stunned_until;target.velocity={};continue;}}
                                target.alive=false;target.velocity={};target.input_x=target.input_y=0;target.respawn_at_us=action_now+kRespawnDelayUs;target.protected_until_us=0;target.shield_until_us=0;target.stunned_until_us=0;++target.round_deaths;++target.session_deaths;if(game::kill_is_awarded(mode,found->second.team,target.team)){++found->second.round_kills;++found->second.session_kills;}emit_sound(players,collision,next_sound_event_id,net::SoundCue::death,found->second.id,target.id,target.position,{found->second.id,target.id});
                            }
                        }
                        if(action!=net::RhythmAction::melee){const auto cue=action==net::RhythmAction::shoot?(result.shot_fired?net::SoundCue::shot_success:net::SoundCue::shot_failure):(result.shield_activated?net::SoundCue::shield_success:net::SoundCue::shield_failure);emit_sound(players,collision,next_sound_event_id,cue,found->second.id,0,found->second.position,{found->second.id},result.shot_fired||result.shield_activated);}
                        found->second.score=result;send_packet(event.peer,net::make_rhythm_result(result),true);
                    }
                }catch(const std::exception & e){std::cerr<<"Rejected packet: "<<e.what()<<"\n";}
                enet_packet_destroy(event.packet);
            }
        }
        constexpr float dt=1.0f/60.0f;const auto now=net::monotonic_time_us();
        for(auto & [peer,player]:players){if(!player.active||!player.alive)continue;const bool movement_locked=game::movement_locked(now,player.shield_until_us,player.stunned_until_us,settings.shield_freeze);Vec2f direction=movement_locked?Vec2f{}:normalize_or_zero({player.input_x/127.0f,player.input_y/127.0f});player.velocity=direction*net::kMoveSpeed;Vec2f candidate=player.position+player.velocity*dt;if(collision.blocks_segment(CollisionChannel::Player,player.position,candidate)){candidate=player.position;player.velocity={};}
...
snapshot.shield_freeze=settings.shield_freeze;
snapshot.shield_melee=settings.shield_melee;
