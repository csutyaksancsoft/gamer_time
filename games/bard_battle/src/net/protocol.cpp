...
SoundEvent read_sound_event(Reader & r){SoundEvent e{};e.id=r.u32();const auto cue=r.u8();if(cue>static_cast<std::uint8_t>(SoundCue::shield_break))fail("Invalid sound cue");e.cue=static_cast<SoundCue>(cue);e.source_id=r.u32();e.target_id=r.u32();e.position={r.f32(),r.f32()};e.server_time_us=r.u64();e.participant=r.u8()!=0;if(!std::isfinite(e.position.x)||!std::isfinite(e.position.y))fail("Non-finite sound event");return e;}
