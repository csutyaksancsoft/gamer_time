...
int main(){auto check=[](bool v){if(!v)throw std::runtime_error("match/ui assertion failed");};
 game::ModeVote vote;const net::PlayerId ids[]={1,2,3};vote.begin(ids,100);check(!vote.complete(101));check(vote.submit(1,net::VoteChoice::teams));check(vote.submit(1,net::VoteChoice::ffa));check(vote.choice(1)==net::VoteChoice::ffa);check(vote.submit(2,net::VoteChoice::ffa));vote.disconnect(3);check(!vote.complete(102));check(vote.result()==net::GameMode::ffa);
